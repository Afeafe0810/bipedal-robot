...
class UpperLevelController(Node):
    ...
    def stance_change(self,state,px_in_lf,px_in_rf,stance,ALIP_count):
        ...
        return stance,self.stance_past
    ...
    def get_initial_data(self,stance):
        '''
        拿切換後以支撐腳的量測值當初始狀態(原本是拿估測值，但其中的扭矩是來自擺動腳)
        感覺X0Y0取值的怪怪的?
        有要取z嗎？可是Psw2com_0只取第一個和第二個？
        '''
        ...
        #藉由支撐狀態切換
        if stance == 1: #(左單支撐)
            #支撐frame #\\\\contact frame
            P_cf_wf = P_L_wf 
            O_wfcf = np.array([[1,0,0],[0,1,0],[0,0,1]])
            #初始狀態
            X0 = copy.deepcopy(self.mea_x_L) #\\\\骨盆的x位置和Ly對L in wf
            Y0 = copy.deepcopy(self.mea_x_R) #\\\\感覺應該是mea_y_L?????????
            #擺動腳前一刻狀態
            O_cfwf = np.transpose(O_wfcf) #\\\\旋轉矩陣的反矩陣是轉置
            Psw2com_X_0 = O_cfwf@(self.P_COM_wf - self.P_R_wf) #\\\\擺動腳2質心的位置在接觸腳
            Psw2com_0 = np.array([[Psw2com_X_0[0,0]],[Psw2com_X_0[1,0]]])#\\\\只取(x,y)
        else:
            #支撐frame
            P_cf_wf = P_L_wf #\\\\\為什麼else還是左腳支撐？
            O_wfcf = np.array([[1,0,0],[0,1,0],[0,0,1]])
            #初始狀態
    ...
    def online_planning(self,stance,contact_t,P_cf_wf,X0,Y0,P_Psw2com_0):
        ...
        #擺動腳參考軌跡
        #更新下一次踩踏瞬間時理想角動量數值
        Ly_T = m*H*l*sinh(l*T)*X0[0,0] + cosh(l*T)*X0[1,0] #\\\\可以用Xx_cf的第二個
        Lx_T = -m*H*l*sinh(l*T)*Y0[0,0] + cosh(l*T)*Y0[1,0]#\\\\可以用Xy_cf的第二個
        #根據下一次支撐腳切換lx
        if stance == 1:
        ...
        pv = t/T #變數 用於連接擺動腳軌跡  #\\\\alpha #\\\\論文上的座標是對wf，這邊之後要轉彎可能要改嗎？？
        ...
        if stance == 1:
            ...
            self.ref_x_L = np.array([[Xx_cf[0,0]],[Xx_cf[1,0]]])
            self.ref_y_L = np.array([[Xy_cf[0,0]],[Xy_cf[1,0]]])
        else:
    ...
    def ref_alip(self,stance,px_in_lf,px_in_rf,com_in_lf,com_in_rf,Com_ref_wf,L_ref_wf,R_ref_wf):
        ...
        L_Y_ref = L_ref_wf[0,1]
        L_Z_ref = L_ref_wf[0,2]

        R_X_ref = R_ref_wf[0,0]
        R_Y_ref = R_ref_wf[0,1]
        R_Z_ref = R_ref_wf[0,2]

        if stance == 1:
            #取骨盆跟質心在位置上的差異(in_wf)
            com2px_in_lf = px_in_lf - com_in_lf
            O_wfL = np.array([[1,0,0],[0,1,0],[0,0,1]]) #直走&理想
            com2px_in_wf = O_wfL@(com2px_in_lf)
            #將質心軌跡更改成骨盆軌跡(in_wf)
            P_X_ref = Com_ref_wf[0,0] + (com2px_in_wf[0,0])
            P_Y_ref = Com_ref_wf[0,1] + (com2px_in_wf[0,1])
            P_Z_ref = 0.55

        elif stance == 0:
            #取骨盆跟質心在位置上的差異(in_wf)
            com2px_in_rf = px_in_rf - com_in_rf
            O_wfR = np.array([[1,0,0],[0,1,0],[0,0,1]]) #直走&理想
            com2px_in_wf = O_wfR@(com2px_in_rf)       
            #將質心軌跡更改成骨盆軌跡(in_wf)
            P_X_ref = Com_ref_wf[0,0] + (com2px_in_wf[0,0])
            P_Y_ref = Com_ref_wf[0,1] + (com2px_in_wf[1,0])
            P_Z_ref = 0.55
    ...
    def calculate_err(self,state):
        ...
        state - copy.deepcopy(state) #\\\\這是什麼
        
         #foot_trajectory(by myself)
        L_ref = LX_ref - PX_ref 
        R_ref = RX_ref - PX_ref

        L = LX - PX #\\\\真實的骨盆2左腳in wf
        R = RX - PX 
        Le = L_ref - L
        Re = R_ref - R
        # --P
        ...
        Lyaw_error_dot = Le_dot[5,0] #\\\\Body_transfer是歐拉角速度轉成幾何嗎
    ...
    def alip_L(self,stance_type,px_in_lf,torque_ALIP,com_in_lf,ALIP_count,state):
        ...
        #質心相對L frame的位置
        PX_l = com_in_wf - lx_in_wf #\\\\L2com_in_wf，為什麼都要加x阿？
        ...
        ref_y_L = copy.deepcopy(self.ref_x_R)
    ...
    def main_controller_callback(self):
        ...
        state,state_past= self.state_collect()
        ...
        if state == 30:##\\\\\\\\\\\\\\\\\就先看到這吧
            if state != state_past or stance != stance_past:
                self.contact_t = 0.0
                self.P_cf_wf,self.X0,self.Y0,self.Psw2com_0 = self.get_initial_data()
            ...
            self.ref_alip(self,stance,px_in_lf,px_in_rf,com_in_lf,com_in_rf,Com_ref_wf,L_ref_wf,R_ref_wf)
