...
class Allcoodinate():
    ...
    def _get_in_wf(self):
        ...
        self.a_pel_in_wf = Rotation.from_matrix(self.r_pel2wf).as_euler('zyx', degrees=False) [::-1] #把旋轉矩陣換成歐拉角zyx,並轉成ax,ay,az
        ...
        (
            (self.p_com_in_wf, self.r_com2wf), #質心
...
class UpperLevelController(Node):
    def __init__(self):
        ...
        self.ref_xLy_com_in_lf = None
        self.ref_xLy_com_in_rf = None
        self.ref_yLx_com_in_lf = None
        self.ref_yLx_com_in_rf = None
    ...
    def main_callback(self):
        ...
        def judge_step_firmly():
            ...
            self.isContact['lf'] == (self.p_lf_in_wf[2,0] <= 0.01)
            self.isContact['rf'] == (self.p_rf_in_wf[2,0] <= 0.01)
        ...
        if self.state == 30:
            torq56 = self.ankle_control()
            torq[self.cf][4:6] = self.estimatorcontrol()
            torq[self.sw][4:6] = torq56[self.sw]
    ...
    def outerloop(self):
        def calculateErr_to_endVeocity() -> dict[str, np.ndarray]:
            ...
            #===========================轉換成geometry端末角速度==================================#
            w_pel2cf_in_wf = zyx_analy_2_gmtry_w(*ref_pa_pel2cf_in_wf[-2:,0]) @ derr_pa_pel2cf_in_wf[-3:] #analytical轉成Geometry
            w_pel2sw_in_wf = zyx_analy_2_gmtry_w(*ref_pa_pel2sw_in_wf[-2:,0]) @ derr_pa_pel2sw_in_wf[-3:]
            ...
            return {
                self.cf : vw_pel2cf_in_wf,
                self.sw : vw_pel2sw_in_wf
            }
        ...
        def Jacobian():
            ...
            for ft in ['l','r']:
                ...
                for i in range(1,6+1): 
                    ...
                    p_ft_in_wf = pt.__dict__[f"p_{self.lf}_in_wf"]
    ...
    def estimatorcontrol(self):
        ...
        return np.vstack(( torq5_cf, torq6_cf ))